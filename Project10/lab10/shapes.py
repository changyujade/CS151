...
class Shape:
    ...
    def __init__(self, distance=100, angle=90, color=(0, 0, 0), lsysString=''):
        ...
        self.turt=ti.TurtleInterpreter()
    ...
    def draw(self, x_pos, y_pos, scale=1.0, heading=0):
        ...
        '''
        self.turt.goto(x_pos, y_pos, heading)
        self.turt.setColor(self.color)
        self.turt.drawString(self.lsysString, self.distance*scale, self.angle)
...
class Pentagon(Shape):
    def __init__(self, distance=100, color=(0, 0, 0), fill=False):
        ...
        # Call the parent's constructor, passing along values for all its
        # parameters.
        Shape.__init__(self,distance, 72, color, lsysString)

def testShape():
    pentagon = Pentagon(color='blue', fill=False)
    triangle = Triangle(color='red', fill=True)
    square = Square(color='sky blue', fill=True)
