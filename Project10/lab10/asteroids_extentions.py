...
class Game:
    ...
    def __init__(self,distance=2,min_x=-200,min_y=0,max_x=200,max_y=200,score=100):
        # two methods. init method that sets up the basic informaiton but does not draw the mosaic 
        # another method is the start screen method that draws 
            #alternatively just use a big rectangle to cover up the screen
        # start game method that would draw background, add player etc. 
        # can use turtle.clear() to clear the screen 
        # turtle background picture .bgpic()
        ...
        self.max_y=max_y
        self.score=score
        print("Welcome to the Game. Your starting score is",score)
        self.makeEnemies()
        self.makeObstacles()
        self.setupEvents()
    ...
    def placeObstacle(self, obstacle,i):
        "Place the enemies at x, y in the canvas"
        #for i in range(4):
        obstacle.speed(0)
    ...
    def moveEnemiesRandomly(self):
        for i in self.enemyList:
            ...
            if (i.xcor()<self.min_x or i.xcor()>self.max_x) and (i.ycor()<self.min_y or i.ycor()>self.max_y):
                i.goto(0,0) #why does it thrown an error here!
    ...
    def checkCollisions(self):
        ...
        if self.score <=0:
            ...
            exit()
        '''
        obstacle_x=self.obstacle.xcor()
        obstacle_y=self.obstacle.ycor() #working on this part
        '''
