class Lsystem:
    ...
    def replace(self, currString):
        ...
        '''
        #currString=self.base #"F++F++F"?
        newString = '' #self.setBase?
        ...
        for char in currString:
            ...
            for rule in self.ruleList:
                if char == rule[0]:
                    ...
                    break
            if not found:
                newString += char #adds the original character
        return newString
    def buildString(self, n):
        ...
        '''
        currString = self.getBase()
        for x in range (n):
            currString = self.replace(currString)
        return currString
