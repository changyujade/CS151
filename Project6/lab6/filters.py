...
def test(filename):
    ...
    # these if statements checks which filter to use when the user types in the command line
    if sys.argv[2]=='swapGreenBlue':
        swapGreenBlue(myImage)
    elif sys.argv[2]=='accentuateRed':
        accentuateRed(myImage)
    elif sys.argv[2]=='lowContrast':
        lowContrast(myImage)
    elif sys.argv[2]=='noRed':
        noRed(myImage)
    elif sys.argv[2]=='blackWhite':
        blackWhite(myImage)
    elif sys.argv[2]=='greenScreen':
        greenScreen('myGreenScreen.ppm','exotic.ppm')
    else:
        win=d.displayImage(myImage,"foliage")
        win.getMouse()
        win.close()
        exit()
    win=d.displayImage(myImage,sys.argv[1])
    win.getMouse()
    win.close()
...
def greenScreen(greenImg, otherImg):
    ...
    '''
    #Enter the command arguments of myGreenScreen.ppm and greenScreen in order to get this part of the code running. 
    greenImg = gr.Image(gr.Point(0, 0), greenImg)
    otherImg= gr.Image(gr.Point(0,0), otherImg)
...
if __name__ == '__main__':
    if len(sys.argv)<2: #checks if the user inputs the right number of command line arguments
